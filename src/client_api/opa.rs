...
impl OpaService for opaService {
    async fn set_opa_policy(
        ...
        let name = std::str::from_utf8(&request.name).unwrap_or_else(|_| {
            error!("parse policyname failed");
            &empty
        });
        let content = std::str::from_utf8(&request.content).unwrap_or_else(|_| {
        ...
        let res = resources::opa::set_policy(name, content)
            ...
            .unwrap_or_else(|e| SetOpaPolicyResponse {
                status: e.into_bytes(),
            });
    ...
    async fn export_opa_policy(
        ...
        let name = String::from_utf8(request.into_inner().name).unwrap_or_else(|_| {
            error!("parse policyname failed");
            "".to_string()
        });
        ...
        let res = resources::opa::export(&name)
            ...
            .unwrap_or_else(|e| ExportOpaPolicyResponse {
                status: e.into_bytes(),
    ...
    async fn set_opa_reference(
        ...
        let res = resources::opa::set_reference(name, content)
            ...
            .unwrap_or_else(|e| SetOpaReferenceResponse {
                status: e.into_bytes(),
            });
    ...
    async fn export_opa_reference(
        ...
        let res = resources::opa::export(&name)
            ...
            .unwrap_or_else(|e| ExportOpaReferenceResponse {
                status: e.into_bytes(),
                content: "".as_bytes().to_vec(),
            });
    ...
    async fn test_opa(
        ...
        if request.policylocal == true {
            policycontent = String::from_utf8(request.policycontent).unwrap_or_else(|_| {
                error!("parse policycontent failed");
                "".to_string()
            });
            ...
            policyname = String::from_utf8(request.policyname).unwrap_or_else(|_| {
                error!("parse policyname failed");
                "".to_string()
            });
        ...
        if request.referencelocal == true {
            referencecontent = String::from_utf8(request.referencecontent).unwrap_or_else(|_| {
            ...
            referencename = String::from_utf8(request.referencename).unwrap_or_else(|_| {
        ...
        let input = String::from_utf8(request.input).unwrap_or_else(|_| {
        ...
        .and_then(|res| serde_json::from_str(&res).map_err(|_| res))
        .and_then(|res: serde_json::Value| Ok(res.to_string()));
